...
# Find closest color value in selected colors palette range 
# 3 — [0, 85, 170, 255]
# 7 — [0,36,72,109,145,182,218,255]
@njit(cache=True)
def find_closest_color(value, colors):
    closest = np.around(colors * value / 255.0) * (255 / colors)
    return closest
...
@njit(cache=True)
def dithering_gray(image_matrix, method, colors=3):
    h = image_matrix.shape[0]
    w = image_matrix.shape[1]

    if method == "Floyd-Steinberg":
        for y, x in np.ndindex(image_matrix.shape):
        ...
        for y, x in np.ndindex(image_matrix.shape):
            old_pixel = image_matrix[y, x]
            old_pixel = value_clip(old_pixel * 0.5 + (old_pixel * bayer8x8[y % 8][x % 8]))
            new_pixel = find_closest_color(old_pixel, colors)
            image_matrix[y, x] = new_pixel
...
@njit(cache=True)
def img_to_raw_frame(input_img, output_frame, colors):
    if colors == 3:
        input_img = input_img >> 6  # Reduce bitdepth to 2 - bit
        for y, x in np.ndindex(input_img.shape):
            for i in range(2):
...
@njit(cache=True)
def bit_frames_to_byte_array(raw_frame, output_byte_array):
    index = 0
    for y in range(0, raw_frame.shape[0], 2):
        for x in range(0, raw_frame.shape[1], 4):
            byte = 0
            for offset in range(0, 4):
                byte += raw_frame[y][x + offset] * (128 >> offset * 2)
                byte += raw_frame[y + 1][x + offset] * (64 >> offset * 2)
